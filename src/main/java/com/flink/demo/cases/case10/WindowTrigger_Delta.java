 ...
 */
@Slf4j
public class WindowTrigger_Delta {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<Row> aggregateStream = streamSourceWithWatermarks.keyBy(0)
                ...
                .trigger(DeltaTrigger.of(5, new DeltaFunction<Row>() {
                    @Override
                    public double getDelta(Row oldDataPoint, Row newDataPoint) {
                        System.out.println("oldDataPoint: " + oldDataPoint);
                        System.out.println("newDataPoint: " + newDataPoint);
                        long old = Long.parseLong(oldDataPoint.getField(5).toString());
                        long ne = Long.parseLong(newDataPoint.getField(5).toString());
                        return ne - old;
                    }
                }, streamSourceWithWatermarks.getType().createSerializer(env.getConfig())))
